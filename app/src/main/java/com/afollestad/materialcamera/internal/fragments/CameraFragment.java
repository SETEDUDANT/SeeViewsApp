...
import com.afollestad.materialcamera.internal.views.CameraPreview;
            ...
            try {
                ...
                mCamera.autoFocus(new Camera.AutoFocusCallback() {
                    ...
                    public void onAutoFocus(boolean success, Camera camera) {
                        ...
                        if (!success) {
                            Log.d(TAG, "onAutoFocus: Unable to auto-focus");
//                            Toast.makeText(getActivity(), "Unable to auto-focus!", Toast.LENGTH_SHORT).show();
    ...
    private boolean prepareMediaRecorder(boolean setVideoSize) {
        try {
            final Activity activity = getActivity();
    ...
    public boolean startRecordingVideo() {
        ...
        try {
            return tryStartRecordingVideo(true);
        } catch (Throwable t) {
            //http://stackoverflow.com/questions/10496969/android-mediarecorder-start-failed-19
            try {
                return tryStartRecordingVideo(false);
            } catch (Throwable t2) {
                t2.printStackTrace();
                mInterface.setRecordingStart(-1);
                stopRecordingVideo(false);
                throwError(new Exception("Failed to start recording: " + t2.getMessage(), t2));
            }
        }
        return false;
    }

    private boolean tryStartRecordingVideo(boolean setVideoSize) {
        if (prepareMediaRecorder(setVideoSize)) {
            try {
                // UI
